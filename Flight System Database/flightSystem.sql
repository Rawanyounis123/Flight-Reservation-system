...
/* Created on:     5/18/2023 12:58:42 AM                        */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('BOOKING') and o.name = 'FK_BOOKING_BOOKING_PASSENGE')
alter table BOOKING
   drop constraint FK_BOOKING_BOOKING_PASSENGE
go
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('SEAT') and o.name = 'FK_SEAT_HAS_FLIGHT')
alter table SEAT
   drop constraint FK_SEAT_HAS_FLIGHT
go
...
create table BOOKING (
   ...
   constraint PK_BOOKING primary key (P_EMAIL, FLIGHT_NUMBER)
)
go
...
create table SEAT (
   ...
   CLASS                text                 not null,
   PRICE                int                  not null,
   constraint PK_SEAT primary key nonclustered (FLIGHT_NUMBER, SEAT_NUMBER)
)
go
...
alter table BOOKING
   add constraint FK_BOOKING_BOOKING_PASSENGE foreign key (P_EMAIL)
      references PASSENGER (P_EMAIL)
go

alter table BOOKING
   add constraint FK_BOOKING_BOOKING2_FLIGHT foreign key (FLIGHT_NUMBER)
...
alter table SEAT
   add constraint FK_SEAT_HAS_FLIGHT foreign key (FLIGHT_NUMBER)
      references FLIGHT (FLIGHT_NUMBER)
go
